// const { FORMS, REPORTS } = require('../../shared/constants');
...
//   const numericValues = results[0]; // Resultados Filtro ADN Físico (Médico)
//   const recomendations = results[1]; // Resultados Recomendaciones ADN Físico (Médico)

//   // TODO: Check if this value is the same for all form results.
//   const offsetIndexes = 4

//   const labels = Object.keys(numericValues).filter((_, index) => {
...
//   Logger.log({ labels, values, maxValues, percentages, wellnessQuotient });
...
//   const formResults = results[FORMS.FORMULARIO_MEDICO_RESPUESTA_A_MEDICAMENTOS] // Resultados Respuesta a Medicamentos (Médico)

//   const offsetIndexes = 4

//   const labels = Object.keys(formResults).filter((_, index) => {
//     return index > offsetIndexes
//   })

//   const values = labels.map(label => parseInt(formResults[label]))
//   const maxValues = labels.map(_ => reportToGenerate.maxValue)
//   const percentages = labels.map((_, index) => (parseInt(values[index]) / maxValues[index]).toFixed(2))
//   const wellnessQuotient = sumArrayValues(values) / sumArrayValues(maxValues)

//   return {
//     date: formResults['Fecha'],
//     labels,
//     maxValues,
//     patientName: formResults['Nombre del Paciente'],
//     percentages,
//     recomendations: [
//       ''
//     ],
...
//   const sintomas = results[0]
//   const saludPhq9 = getValues(results[1], offsetIndexes)
//   const estresPercibido = getValues(results[2], offsetIndexes)
//   const transtornoAnsiedad = getValues(results[3], offsetIndexes)
...
//     recomendations: [
//       // recomendacionesAdnMente || null,
//       // recomendacionesAdnFisico || null
//     ],
//     wellnessQuotient,
